...
public class OutrageEffect extends StatusEffect {
    ...
    public void onApplied(LivingEntity entity, AttributeContainer attributes, int amplifier) {
        ...
        if (entity.hasStatusEffect(BerserkerEffects.RAGE.effect)) {
            var current = entity.getStatusEffect(BerserkerEffects.OUTRAGE.effect);
            if (current != null) {
                int bonusTicks = Math.round(tweaksConfig.value.outrage_rage_bonus_duration * 20F);
                APPLYING_RAGE_BONUS.add(entity.getUuid());
                entity.addStatusEffect(new StatusEffectInstance(BerserkerEffects.OUTRAGE.effect, current.getDuration() + bonusTicks, amplifier));
            }
        }
    }
}
